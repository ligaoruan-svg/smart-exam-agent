          ...
          <div class="msg-inner">
            ...
            <div class="msg-body">
              ...
              <div v-for="(card, ci) in (m.cards || [])" :key="`c-${ci}`" class="ui-card-wrap">
                ...
                <!-- study_report 卡片 -->
                <div
                  v-else-if="card.type === 'study_report'"
                  class="card-study-report"
                >
                  <div class="sr-head">
                    <span class="sr-icon">📊</span>
                    <span class="sr-title">学情报告</span>
                  </div>
                  <div class="sr-body">
                    <div class="sr-row">
                      <span>总答题</span>
                      <b>{{ card.data?.total || 0 }} 题</b>
                    </div>
                    <div class="sr-row">
                      <span>正确率</span>
                      <b>{{ card.data?.correct_rate || 0 }}%</b>
                    </div>
                    <div v-if="card.data?.weakest" class="sr-row">
                      <span>最弱模块</span>
                      <b class="sr-warn">{{ card.data.weakest }}</b>
                    </div>
                    <div v-if="!card.data?.has_data" class="sr-tip">
                      📌 答题量还不够生成模块分析，继续刷题吧
                    </div>
                  </div>
                </div>
...
.sr-row span { color: #6b7280; }
.sr-row b { color: #111827; font-weight: 600; }
.sr-warn { color: #dc2626 !important; }
.sr-tip {
  margin-top: 10px;
  padding: 8px 12px;
  background: #fef3c7;
